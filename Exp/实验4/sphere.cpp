...
void Sphere::init(int prec) {
	...
	for (int i = 0; i < numVertices; i++) { vers_normals.push_back(glm::vec3()); }
	...
	for (int i = 0; i <= prec; i++) {
		for (int j = 0; j <= prec; j++) {
			...
			vertices[i*(prec + 1) + j] = glm::vec3(r*x, r*y, r*z);
			texCoords[i*(prec + 1) + j] = glm::vec2(((float)j / prec), ((float)i / prec));
			normals[i*(prec + 1) + j] = glm::vec3(r*x, r*y, r*z);
			vers_normals[i*(prec + 1) + j] = glm::vec3(-r*x, -r*y, -r*z);
